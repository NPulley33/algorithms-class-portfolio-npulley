...
- Searching Algorithms:
  - Linear Search: O(n). Sequentially iterates though and checks all elements of an array.
  - Binary Search: O(log n). Only works with a sorted array. Checks value in the middle of the search set. If the target value is greater, the search space is halved to the greater half. If the target value is less, the search space is halved to the lesser half.
  - Interpolation Search O(log log n). Only works with a sorted array. Works best with uniformly distributed data. Uses a calculated probe value to estimate where the target value might be. 
...
- Maps vs. Arrays:
  ...
  - Maps are useful when you need to worry about space and accessing elements quickly. Arrays are useful for any kind of traversals, shuffling of data, and sorting of data sets. 
- Stacks vs. Queues:
